...
def run_seed_doctors():
    ...
    # Specialty mapping with some symptoms for generation
    specialties = {
        1: {"name": "Nội tổng quát", "symptoms": "Sốt, đau đầu, mệt mỏi, khám sức khỏe định kỳ"},
        2: {"name": "Tim mạch", "symptoms": "Đau ngực, huyết áp, loạn nhịp tim, khó thở"},
        3: {"name": "Da liễu", "symptoms": "Mẩn ngứa, dị ứng, mụn nhọt, rụng tóc"},
        4: {"name": "Cơ xương khớp", "symptoms": "Đau lưng, đau khớp, thoái hóa, gout"},
        5: {"name": "Tiêu hóa", "symptoms": "Đau dạ dày, tiêu chảy, táo bón, ợ hơi"},
        6: {"name": "Tai Mũi Họng", "symptoms": "Viêm họng, viêm xoang, ù tai, nghẹt mũi"},
        7: {"name": "Thần kinh", "symptoms": "Đau đầu, chóng mặt, mất ngủ, rối loạn tiền đình"},
        8: {"name": "Nhi khoa", "symptoms": "Sốt, ho, biếng ăn, tiêm chủng trẻ em"},
        9: {"name": "Sản phụ khoa", "symptoms": "Khám thai, rối loạn kinh nguyệt, phụ khoa"},
        10: {"name": "Nhãn khoa", "symptoms": "Cận thị, đục thủy tinh thể, viêm kết mạc"},
        11: {"name": "Ung bướu", "symptoms": "Tầm soát ung thư, khối u, xạ trị"},
        12: {"name": "Nam khoa", "symptoms": "Rối loạn cương dương, hiếm muộn, viêm nhiễm nam khoa"},
        13: {"name": "Nha khoa", "symptoms": "Sâu răng, nhổ răng, niềng răng, viêm nướu"},
        14: {"name": "Thận - Tiết niệu", "symptoms": "Sỏi thận, tiểu đêm, viêm đường tiết niệu"},
        15: {"name": "Hô hấp", "symptoms": "Hen suyễn, viêm phế quản, COPD, ho kéo dài"},
        16: {"name": "Chấn thương chỉnh hình", "symptoms": "Gãy xương, bong gân, chấn thương thể thao"},
        17: {"name": "Phục hồi chức năng", "symptoms": "Vật lý trị liệu, phục hồi sau tai biến"},
        18: {"name": "Y học cổ truyền", "symptoms": "Châm cứu, bấm huyệt, bốc thuốc nam/bắc"},
        19: {"name": "Nội tiết", "symptoms": "Tiểu đường, tuyến giáp, rối loạn chuyển hóa"},
        20: {"name": "Truyền nhiễm", "symptoms": "Sốt xuất huyết, viêm gan, bệnh nhiệt đới"},
        21: {"name": "Dị ứng - Miễn dịch", "symptoms": "Dị ứng thời tiết, dị ứng thuốc, lupus"},
        22: {"name": "Dinh dưỡng", "symptoms": "Tư vấn chế độ ăn, giảm cân, suy dinh dưỡng"}
    }
    
    # Names for generation
    last_names = ["Nguyễn", "Trần", "Lê", "Phạm", "Hoàng", "Huỳnh", "Phan", "Vũ", "Võ", "Đặng", "Bùi", "Đỗ", "Hồ", "Ngô", "Dương", "Lý"]
    middle_names = ["Thị", "Văn", "Hữu", "Minh", "Đức", "Ngọc", "Xuân", "Thanh", "Quang", "Đình", "Hải", "Tuấn", "Thành", "Hoài", "Quỳnh"]
    first_names = ["Anh", "Tuấn", "Linh", "Trang", "Minh", "Cường", "Hương", "Lan", "Hùng", "Hải", "Dũng", "Thảo", "Phương", "Bình", "Châu", "Yến", "Oanh", "Khang", "Tâm", "Hòa"]
    
    import random
    random.seed(42)
    
    # Remove existing doctors from users table
    print("Cleaning existing doctors...")
    users = users[users["role"] != "doctor"].copy()
    ...
    doc_id = 1
    user_id = int(users["id"].max()) + 1 if not users.empty else 1
    
    print("Generating doctors...")
    for _, clinic in clinics.iterrows():
        clinic_id = int(clinic["id"])
        spec_ids_str = str(clinic["specialty_ids"])
        spec_ids = [int(s) for s in spec_ids_str.split(",") if s.strip().isdigit()]
        
        for spec_id in spec_ids:
            if spec_id not in specialties:
                continue
                
            # Generate 1 to 2 doctors per specialty per clinic
            num_docs = random.choice([1, 2])
            for _ in range(num_docs):
                name = f"{random.choice(last_names)} {random.choice(middle_names)} {random.choice(first_names)}"
                email = f"bs.c{clinic_id}.s{spec_id}.{doc_id}@medibook.vn"
                phone = f"09{random.randint(10000000, 99999999)}"
                cccd = f"001{random.randint(100000000, 999999999)}"
                exp_years = random.randint(3, 30)
                rating = round(random.uniform(4.0, 5.0), 1)
                
                spec_info = specialties[spec_id]
                ...
                new_doc = {
                    ...
                    "rating": rating,
                    "symptoms": spec_info["symptoms"],
                    "email": email
                }
                new_doctors.append(new_doc)
                ...
                new_user = {
                    ...
                    "dob": f"{random.randint(1960, 1995)}-01-01"
                }
    ...
    print(f"Generated {len(new_doctors)} doctors.")
    
    # Save DataFrames
    new_docs_df = pd.DataFrame(new_doctors)
    new_users_df = pd.concat([users, pd.DataFrame(new_users)], ignore_index=True)
    
    new_docs_df.to_csv("data/doctors.csv", index=False)
    new_users_df.to_csv("data/users.csv", index=False)
    
    print("Saved doctors.csv and users.csv successfully.")
